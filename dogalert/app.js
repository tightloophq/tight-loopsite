...
function ok(el, text) { el.textContent = text; el.style.color = "#9be79b"; }
function bad(el, text) { el.textContent = text; el.style.color = "#ff9b9b"; }
    ...
    onSnapshot(q, (snap) => {
      ...
      snap.docChanges().forEach((ch) => {
        ...
        if (!markers.get(id)) {
          ...
          const f = markers.get(id);
          f.data = d; f.marker.setPosition(pos);
        }
      });

      markers.forEach(() => visible++);
      statusPins.textContent = String(visible);
...
form.addEventListener("submit", async (e) => {
  ...
  try {
    ...
    bad(statusDb, "write error ❌ (rules/billing?)");
    // keep temp marker visible so the user still sees feedback
  }
});
